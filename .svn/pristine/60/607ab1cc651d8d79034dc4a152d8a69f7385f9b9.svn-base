        ...
        <mescroll-vue ref="mescroll"  :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
          <div class="seller-head" :style="{'background-image':slides ? 'url('+slides+')' :'url(./static/libs/img/sjbj.png)'}" ref="mate">
              ...
              <div class="shop-class">
               <span v-for="(item,index) in shop.category" :class="{'pink-text':tab==index}" @click="switchTab(index,item.id)" :key="index">{{item.title}}</span>
              </div>
              <!--<div class="shop-name">
  ...
  methods: {
    ...
    getShopInfo () {
      ...
      apiHttp.shopInfo(id).then(response => {
        if (response.code === 0) {
          this.isShow = true
          this.spid = response.data.shopInfo.id
          this.shopId = response.data.shopInfo.category[0].id
    ...
    upCallback (page, mescroll) {
      if (this.select === 1) {
        this.$http.post('/mall/index/goods-list',{page: page.num, pcid : this.shopId,spid:this.spid}).then(res => {
          if (res.code === 0) {
